...
#section2 ul li{ display: block; float: left; padding: 40px; margin-right: 30px }
...
<div id="section">
	...
	<div id="catrgory"> ACCESSORIES </div>

<table>
		<c:forEach var="acclist" items="${acclist}">
		<tr>
			<td>
			<c:set var="code" value="${acclist.p_code}"/>
			<a href="/thk/product_1.cp?p_code=${code}"><img src="/thk/save/${acclist.p_thumbnail}"  height="230" ></a>
			<div id="productinfo">
				<ul>
					<li style="font-size:15px">${acclist.p_name}</li>
					<li style="font-size:13px; text-decoration: line-through;">
					<fmt:formatNumber value="${acclist.p_price_dc}" pattern="#,##0"/>원</li>
					<li style="font-size:20px">
					<fmt:formatNumber value="${acclist.p_price_dc}" pattern="#,##0"/>원</li>
					<li>
					 <c:if test="${acclist.result == 1}">
					    <img src="/thk/save/icon/new.png" width="50">
					    </c:if>
					  <c:if test="${acclist.readcount >=10 }">
