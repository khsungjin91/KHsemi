...
tr {float:left; width:320px; margin: 5px; border: 2px solid black; }
...
<c:if test="${searchlist.img != null}">
			<a href="/thk/reviewContent.cp?num=${searchlist.num}"><img src="/thk/save/${searchlist.img}" width="290" height="200" style="margin-left:15px " ></a>
</c:if>
<c:if test="${searchlist.img == null}">
...
<div id="reviewinfo">
<ul>
				<li><a href="/thk/reviewContent.cp?num=${searchlist.num}" style="text-decoration: none; color: black;">
				${searchlist.subject} </a>
					<c:if test="${relist.readcount >= 20}">
					<img src="/thk/save/icon/best.png" width="40">
					</c:if>
				</li>
				<li>상품이름  &nbsp; ${searchlist.p_name}</li>
				<li>작성자 &nbsp; ${searchlist.mem_id}</li>
				<li>조회수 &nbsp; ${searchlist.readcount}</li>
				<li>상품평점  &nbsp; ${searchlist.star}</li>
			</ul>
		</div>
		</td>
	</tr>
...
<c:if test="${Count > 0}">
	...
	<a href = "/thk/search.cp?Pnum=${startPage-10}&item=${item}&search=${search}"> &lt; </a>
	</c:if>
	
	<c:forEach var="i" begin="${startPage}" end="${endPage}">
	<a href = "/thk/search.cp?Pnum=${i}&item=${item}&search=${search}">[${i}]</a> 
	</c:forEach>
