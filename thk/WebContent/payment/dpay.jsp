...
<div id="section">
  ...
  <div align=center>일 매출(등록 갯수:${count})</div>
...
<c:if test="${count > 0 }">
  ...
  <a href="/thk/dpay.cp?pageNum=${startPage - 10 }">[이전]</a>
  </c:if>

  <c:forEach var="i" begin="${startPage}" end="${endPage}">
    <a href="/thk/dpay.cp?pageNum=${i}">[${i}]</a>
  </c:forEach>

  <c:if test="${endPage< pageCount }">
    <a href="/thk/dpay.cp?pageNum=${startPage + 10}">[다음]</a>
  </c:if>
</c:if>
<br/>
