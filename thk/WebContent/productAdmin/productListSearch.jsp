...
<table width="1200">
  ...
  <tr>
    ...
    <td align=center>${article.p_price}</td>
    <td align=center>${article.p_price_dc}</td>
...
<c:if test="${count > 0 }">
  ...
  <a href="/thk/productListSearch.cp?pageNum=${startPage - 10 }&search=${search}&item=${item}">[이전]</a>
  </c:if>

  <c:forEach var="i" begin="${startPage}" end="${endPage}">
    <a href="/thk/productListSearch.cp?pageNum=${i}&search=${search}&item=${item}">[${i}]</a>
  </c:forEach>

  <c:if test="${endPage< pageCount }">
    <a href="/thk/productListSearch.cp?pageNum=${startPage + 10}&search=${search}&item=${item}">[다음]</a>
  </c:if>
</c:if>
